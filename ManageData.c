                    ...
                    {   
                        ...
                        system("clear");    
                                ...
                                {                
                                        ...
                                        if (subchoice<6 && subchoice>=0)
                                        {
                                            printf("date : %s\n",dateToString(dateArchive));
                                            display(student,subchoice,dateToString(dateArchive));
                                            b=subchoice;
                                        ...
                                        {   
                                            system("clear");
                                            FileGenerator(student,b,dateToString(dateArchive));
                                            a=-1;
                                        }
                                        if (subchoice !=7)
                                        {
...
{	
    ...
    while (1)
     {  
        t=0;
        printf("\n\n\n\t\t🔴 Entrer the number or the name or the prename of the student or tap 'quit' to quit:");
...
void Save_for_human(Machine *eleve)
{
 FILE *file = Open(FILE_NAME_Saveforhuman,"a+");
    ...
    fprintf(file,"<HTML>\n<HEAD>\n<TITLE>Liste</TITLE>\n</HEAD>\n<BODY>");
    fprintf (file,"<BODY>Date:%s\n",initializeLaunchDate());
    fprintf(file, "<TABLE >\n<TR>\n<TD>%-*s</TD><TD>%-*s</TD><TD>%-*s</TD><TD>%*s</TD><BR></TR>",WIDTH_num, "NUM",WIDTH_name, "NAME", WIDTH_prenames, "PRENAMES", WIDTH_DATA, "DATA");
    fprintf(file,"|%s\n","TIME");
    // Écrire les données
    for (i = 0; i < STUDENT_N; i++) 
    {
        ...
        fprintf(file, "<TD>%-*s</TD><TD>%02d:%02d:%02d</TD</TR><BR>", WIDTH_DATA, eleve[i].presence, eleve[i].t.tm_hour, eleve[i].t.tm_min, eleve[i].t.tm_sec);
    }
    Countstudent(eleve,&abs,&oui,&non,&perso,&pres);
    fprintf(file,"</TABLE>");
    fprintf(file,"Total number of student:%d<BR>",STUDENT_N);
    fprintf(file,"Present number:%d\tAbsent number:%d<BR>Student number depending on usage type:\n",pres,abs);
